...
def noise(img, noise_quantity, noise_deviation):
    ...
    for numpix in range(0, noise_quantity*width*height//100):
        # Случайный пиксель
        x=random.randint(2, img.size[0]-2)
        y=random.randint(2, img.size[1]-2)
        # Узнаем яркость
        ...
        for x_1 in range(x, x + random.randint(0, 2)):
            for y_1 in range(y, y + random.randint(0, 2)):
